...
public partial class _Employee_Vacation : basePageSessionExpire
{
    ...
    public static string getadminusersData()
    {
        ...
        using (
            SqlCommand cmd =
                new SqlCommand("Select * from adminusers where Dept_id = @Dept_id and Emp_ID <> @Emp_id",
                    con)
        )
        {
            ...
            //cmd.Parameters.AddWithValue("@Emp_id", Convert.ToInt32(HttpContext.Current.Session["Emp_ID"]));
            //cmd.Parameters.AddWithValue("@Dept_id", HttpContext.Current.Session["Dept_id"]);
    ...
    public static string gettypesData()
    {
        ...
        using (
            SqlCommand cmd =
                new SqlCommand("Select id,Type_Name from HR_Vacation_Type where Hide <> 'True'",
                    con)
        )
        {
            SqlDataReader idr = cmd.ExecuteReader();
    ...
    // getDaysOffData
    [WebMethod]
    public static string getDaysOffData()
    {
        string config =
            Convert.ToString(ConfigurationManager.ConnectionStrings["dbcon"]);
        ...
        using (
            SqlCommand cmd =
                /* new SqlCommand("Select * from HR_DaysOFF_Requests where ExpirationDate > GETDATE() and Canceled <> 'True' and Status = 4 and Emp_id =" +
                    HttpContext.Current.Session["Emp_ID"],
                    con)*/
                new SqlCommand("Select * from HR_DaysOFF_Requests where ExpirationDate > GETDATE() and Canceled <> 'True' and Status = 4 and Emp_id = @Requester_id",
                    con)
        )
        {
    ...
    public static List<daysOff>
    populatedaysOffLisst(SqlDataReader idr, SqlConnection con)
    {
    ...
    [WebMethod]
    public static string getPeriodsData()
    {
        string config =
            Convert.ToString(ConfigurationManager.ConnectionStrings["dbcon"]);
        ...
        using (
            SqlCommand cmd =
                new SqlCommand("SELECT Date_From FROM HR_Vacations_Period WHERE Emp_code = @Emp_id and Request_id IN (SELECT id FROM HR_Vacations_Requests WHERE Vacation_Type_name = 'Casual' and Status = 4)",
                    con)
        )
        {
            cmd
                .Parameters
                .AddWithValue("@Emp_id", HttpContext.Current.Session["Emp_ID"]);
            SqlDataReader idr = cmd.ExecuteReader();
    ...
    public static string NewHR_Vacations_Days(Vacations_Days Vacations_Days)
    {
        ...
        using (
            SqlCommand cmd =
                new SqlCommand("insert into HR_Vacations_Period (Request_id,Emp_code,Type_id,Date_from,Date_to) values (@Request_id,@Emp_code,@Type_id,@Date_from,@Date_to)",
                    con)
        )
        {
            cmd.Parameters.Add("@Request_id", SqlDbType.Int).Value = NewId;
            cmd.Parameters.Add("@Emp_code", SqlDbType.Int).Value =
                HttpContext.Current.Session["Emp_ID"];
            cmd.Parameters.Add("@Type_id", SqlDbType.Int).Value = Type_id;
            cmd.Parameters.Add("@Date_from", SqlDbType.VarChar).Value =
                Vacations_Days.Date_from;
            cmd.Parameters.Add("@Date_to", SqlDbType.VarChar).Value =
                Vacations_Days.Date_to;
            cmd.ExecuteNonQuery();
        }
    ...
    public static string checkBalance()
    {
        ...
        )
        {
            cmd
                .Parameters
                .AddWithValue("@Emp_id",
                Convert.ToInt32(HttpContext.Current.Session["Emp_ID"]));
            SqlDataReader idr = cmd.ExecuteReader();
    ...
    public static List<cats>
    populatecatsLisst(SqlDataReader idr, SqlConnection con)
    {
                ...
                .Add(new cats {
                    id =
                        idr["id"] != DBNull.Value
                            ...
                            : 0,
                    Name =
                        idr["Name"] != DBNull.Value
                            ? Convert.ToString(idr["Name"])
                            : String.Empty,
                    Icon =
                        idr["Icon"] != DBNull.Value
                            ? Convert.ToString(idr["Icon"])
                            : String.Empty
                });
        }
