...
public partial class _Manger : System.Web.UI.Page
{
                ...
                .Add(new vacations {
                    ...
                    Vacation_Type_name =
                        idr["Vacation_Type_name"] != DBNull.Value
                            ...
                            : String.Empty,
                    Delegation_EmpCode =
                        idr["Delegation_EmpCode"] != DBNull.Value
                            ? Convert.ToInt32(idr["Delegation_EmpCode"])
                            : 0,
                    Delegation_EmpName =
                        idr["Delegation_EmpName"] != DBNull.Value
                            ? Convert.ToString(idr["Delegation_EmpName"])
                            : String.Empty,
                    ...
                    Hide =
                        idr["Hide"] != DBNull.Value
                            ...
                            : false,
                    Hide_Manager =
                        idr["Hide_Manager"] != DBNull.Value
                            ? Convert.ToBoolean(idr["Hide_Manager"])
                            : false,
                    OFF_Day =
                ...
                .Add(new requests {
                    ...
                    HR_Comment =
                        idr["HR_Comment"] != DBNull.Value
                            ...
                            : String.Empty,
                    Canceled =
                        idr["Canceled"] != DBNull.Value
                            ? Convert.ToBoolean(idr["Canceled"])
                            : false,
                    Manager_Hide =
                        idr["Manager_Hide"] != DBNull.Value
                            ? Convert.ToBoolean(idr["Manager_Hide"])
                            : false,
                    Entry_date = Convert.ToDateTime(idr["Entry_date"])
                });
        }
    ...
    {
        List<HR_Employees_Vacation_Balance> HR_Employees_Vacation_BalanceI =
            new List<HR_Employees_Vacation_Balance>();

        while (idr.Read())
        {
            HR_Employees_Vacation_BalanceI
    ...
    public static string updateStatus(StatusDetails detail)
    {
        ...
        using (
            SqlCommand cmd =
                new SqlCommand("update HR_DaysOFF_Requests set Status= @Status where id = @id;",
                    con)
        )
        {
    ...
    public static string Rejection(HR_DaysOFF_Requests request)
    {
        ...
        SqlCommand cmd =
            new SqlCommand("update HR_DaysOFF_Requests set Manager_Comment= @Manager_Comment, Status= @Status where id = @id;",
                con);
        {
            cmd
                .Parameters
                .AddWithValue("@Manager_Comment", request.Manager_Comment);
    ...
    public static string VacRejection(HR_Vacations_Requests request)
    {
        ...
        SqlCommand cmd =
            new SqlCommand("update HR_Vacations_Requests set Manager_Comment= @Manager_Comment, Status= @Status  where id = @id;",
                con);
        {
            cmd
                .Parameters
    ...
    public static string updateHide(HideDetails detail)
    {
        ...
        using (
            SqlCommand cmd =
                new SqlCommand("update HR_Vacations_Requests set Hide_Manager= @Hide_Manager where id = @id;",
                    con)
        )
        {
            cmd.Parameters.AddWithValue("@id", detail.id);
    ...
    public static string updateWorkHide(HideWorkDetails detail)
    {
        ...
        using (
            SqlCommand cmd =
                new SqlCommand("update HR_DaysOFF_Requests set Manager_Hide= @Manager_Hide where id = @id;",
                    con)
        )
        {
            cmd.Parameters.AddWithValue("@id", detail.id);
